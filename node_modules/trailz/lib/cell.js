...
class Cell {
     ...
     */
    get repr() {
        let val = 15; // Default all 4 sides "on"
        if (this.linked(this._north))
            val &= 7; // turn off "top"
        if (this.linked(this.east))
            val &= 11; // turn off "right"
        if (this.linked(this.south))
            val &= 13; // turn off "bottom"
        if (this.linked(this.west))
            val &= 14; // turn off "left"
        return val;
    }
    /**
    ...
    get visitedNeighbors() {
        const l = [];
        if (this._north && !this._north.linksEmpty)
            l.push(this._north);
        if (this._south && !this._south.linksEmpty)
            l.push(this._south);
        if (this._east && !this._east.linksEmpty)
            l.push(this._east);
        if (this._west && !this._west.linksEmpty)
            l.push(this._west);
        return l;
    }
    /**
